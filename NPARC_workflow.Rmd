...
repeatFits <- function(x, y){
  ...
  while (doFit){
    ...
    # doFit <- !m$convInfo$isConv & i < 100
    doFit <- inherits(m, "try-error") & i < 100
    
    doVaryPars <- TRUE
  }
  
  # if (!m$convInfo$isConv & i == 100){
  #   class(m) <- "try-error"
  # }
...
returnPredictions <- function(dat){
  ...
  # Fit model:
  fit <- repeatFits(x = dat$temperature, 
                    y = dat$relAbundance)
  ...
  if (!inherits(fit, "try-error")){
    ...
    # Obtain data frame with predicted values and residuals:
    out <- broom::augment(fit) %>%
      ...
      # Rename generic columns `x`. `y` that were added by the augment function 
      # to `temperature` and `relAbundance`:
      dplyr::rename(temperature = x, relAbundance = y)
    
  } else {
    # If model fit did not converge, return an empty data frame.
    out <- tibble(temperature = dat$temperature, 
                  relAbundance = NA,
...
nullPredictions <- tppData %>% #filter(uniqueID == "ABHD10_NA", dataset == "Panobinostat") %>%
...
alternativePredictions <- tppData %>%
  ...
  do(
    # Invoke model fitting as described above
    returnPredictions(.)
    ) %>%
  ungroup
```
...
Let us look at the columns of the data frame `allPredictions`:
...
rssPerModel <- allPredictions %>%
  group_by(dataset, uniqueID) %>%
  summarise(fittedValues = sum(!is.na(nullResiduals) * !is.na(alternativeResiduals)), 
            rssNull = sum(nullResiduals^2, na.rm = FALSE),
            rssAlternative = sum(alternativeResiduals^2, na.rm = FALSE)) %>%
  ungroup
```
...
Proteins for which $\operatorname{RSS}^1$ is not smaller than $\operatorname{RSS}^0)$ are excluded from p-value calculation.
```{r filterRSS}
rssFiltered <- rssPerModel %>% filter(rssDiff > 0)
```

For how many proteins could be compute RSS differences per dataset?
```{r countRSS}
rssFiltered %>%
  group_by(dataset) %>%
  do(table(.$fittedValues) %>% as.tibble() %>% dplyr::rename(fittedValues = Var1)) %>%
  knitr::kable()
```


Let us look at the first entries and at the column summaries of the data frame `rssPerModel`:
...
```{r summary_rssPerModel}
rssFiltered %>%
  mutate(dataset = factor(dataset),
         fittedValues = factor(fittedValues)) %>%
  summary(digits = 2) %>% 
  knitr::kable()
```
...
<!--   geom_point(aes(shape = factor(replicate), color = factor(molarDrugConcentration))) + -->
<!--   geom_line(aes(y = alternativePrediction, color = factor(molarDrugConcentration))) + -->
<!--   theme_bw() + -->
<!--   ggtitle("RRP9_NA") + -->
...
DOF <- rssFiltered %>%
  mutate(paramsNull = 3,
         paramsAlternative = ifelse(fittedValues > 40, yes = 9, no = 6)) %>%
  mutate(DOF1 = paramsAlternative - paramsNull,
         DOF2 = fittedValues - paramsAlternative)
```

Let us take a look at the computed degrees of freedom:
```{r showDOFiid}
DOF %>% 
  distinct(dataset, fittedValues, paramsNull, paramsAlternative, DOF1, DOF2) %>%
  knitr::kable()
```
...
```{r computeFiid}
fStatistics <- DOF %>%
  mutate(fStat = (rssDiff/DOF1) / (rssAlternative/DOF2),
...
ggplot(fStatistics) +
  ...
  facet_wrap(~ dataset + fittedValues) +
  theme_bw() +
...
scalingFactors <- rssFiltered %>%
  group_by(dataset) %>%
  summarise(M = median(rssDiff, na.rm = T), V = mad(rssDiff, na.rm = T)^2) %>%
  ungroup %>%
  mutate(s0_sq = 1/2 * V/M)

scalingFactors %>% knitr::kable(digits = 4)
```

We scale the numerator and denominator in Equation (\ref{eq:f_stat}) by these scaling factors and estimate the degree of freedom parameters by fitting unscaled $\chi^2$ distributions.
...
rssScaled <- scalingFactors %>%
  ...
  left_join(rssFiltered, by = "dataset") %>%
  mutate(rssDiff = rssDiff/s0_sq,
         rssAlternative = rssAlternative/s0_sq)
```
...
newDOF <- rssScaled %>%
  ...
  do(
    data.frame(
      ...
      DOF2 = MASS::fitdistr(x = .$rssAlternative, densfun = "chi-squared", start = list(df = 1))[["estimate"]]
    ))
```
...
Let us look at the targets detected for each compound treatment. These are the same proteins whose melting curves are shown in Fig. S3, S4, S6, and S7.
...
# Compare to the Tm-based approach
