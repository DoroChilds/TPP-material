...
plt <- ggplot(comparison) +
  geom_point(aes(rssNullPaper, y = abs(rssNullPaper - rssNull), key = uniqueID)) +
  facet_wrap(~ dataset) +
  # coord_equal() +
  scale_x_log10() +
  # scale_y_log10() +
...
rssFiltered <- rssPerModel %>%
  group_by(dataset)# %>%
  # filter(rssDiff <= quantile(rssDiff, 0.99, na.rm = TRUE))
...
scalingFactors %>% knitr::kable(digits = 9)
```
