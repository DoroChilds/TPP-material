...
DOF <- rssFiltered %>%
  mutate(paramsNull = 3,
         paramsAlternative = ifelse(n > 40, yes = 9, no = 6)) %>%
  mutate(DOF1 = paramsAlternative - paramsNull,
         DOF2 = n - paramsAlternative)
```

Let us take a look at the computed degrees of freedom:
```{r}
DOF %>% 
  distinct(dataset, n, paramsNull, paramsAlternative, DOF1, DOF2) %>%
  knitr::kable()
```
